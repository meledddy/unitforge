...
const loginThemeStorageKey = "unitforge-login-theme";
    ...
    >
      ...
      <div className="relative z-10 mx-auto flex min-h-screen w-full max-w-[1180px] flex-col px-5 py-5 sm:px-8 sm:py-8">
          ...
          <Link className="marketing-focus-ring rounded-full" href="/">
            <UnitforgeLogo
              ...
              wordmarkClassName={cn(
                ...
                loginTheme === "light" && "text-[hsl(var(--login-logo-wordmark))]",
              )}
            />
          </Link>
          <div className="flex items-center gap-2">
            <LoginThemeToggle mounted={isThemeMounted} theme={loginTheme} onToggle={toggleLoginTheme} />
          ...
          <div className="relative w-full max-w-[430px] text-center">
            <div className="relative mx-auto flex flex-col items-center">
              ...
              <div className="mt-7 space-y-2">
                ...
                <p className="text-sm leading-6 text-[hsl(var(--login-foreground-muted))] sm:text-[0.96rem]">{messages.auth.formSubtitle}</p>
              </div>
              <div className="mt-7 w-full animate-in fade-in slide-in-from-bottom-2 duration-700 motion-reduce:animate-none">
                <SignInForm locale={locale} next={next} showBackLink={false} showHeader={false} />
              </div>
              <Link
              ...
              >
                <span className="flex h-9 w-9 items-center justify-center rounded-full border border-[hsl(var(--login-border)/0.72)] bg-[hsl(var(--login-surface-muted)/0.52)] text-sm font-medium text-[hsl(var(--login-foreground-soft))]">
                  {appConfig.name.charAt(1)}
                </span>
                <span>{messages.auth.returnToPublicSite}</span>
                <span aria-hidden="true" className="text-[hsl(var(--login-accent))]">
                  -&gt;
                </span>
              </Link>
            </div>
        ...
        <footer className="flex items-center justify-center pb-3 text-xs text-[hsl(var(--login-foreground-muted)/0.78)]">
          <span>(c) 2026 {appConfig.name}. All rights reserved.</span>
        </footer>
      </div>
    </section>
  );
...
function FoldedUnitforgeMark({ theme }: { theme: LoginTheme }) {
  return (
    <div className="relative h-36 w-36 sm:h-40 sm:w-40">
      <div
        className={cn(
          "absolute left-1/2 top-1/2 h-60 w-60 -translate-x-1/2 -translate-y-1/2 rounded-full bg-[radial-gradient(circle,hsl(var(--login-mark-cream)/0.22)_0%,hsl(var(--login-mark-glow-core)/0.28)_24%,hsl(var(--login-mark-glow-soft)/0.14)_46%,transparent_72%)] blur-2xl motion-safe:animate-pulse",
          theme === "light" &&
      ...
      <div
        className={cn(
          ...
          theme === "light" && "bg-[radial-gradient(ellipse,hsl(var(--login-mark-glow-core)/0.16),transparent_68%)]",
        )}
      />
      <div
        className={cn(
          ...
          theme === "light" && "bg-[radial-gradient(circle,hsl(var(--login-mark-cream)/0.14),transparent_66%)]",
        )}
      />
      <svg aria-hidden="true" className="relative h-full w-full drop-shadow-[0_0_34px_hsl(var(--login-mark-glow-core)/0.58)]" fill="none" viewBox="0 0 160 160">
        <defs>
          <linearGradient id="login-fold-left" x1="34" x2="83" y1="28" y2="134" gradientUnits="userSpaceOnUse">
          ...
          <linearGradient id="login-fold-right" x1="126" x2="78" y1="28" y2="134" gradientUnits="userSpaceOnUse">
          ...
          <linearGradient id="login-fold-core" x1="80" x2="80" y1="41" y2="139" gradientUnits="userSpaceOnUse">
        ...
        <path
          ...
          strokeWidth="1"
        />
        <path d="M80 45V136" stroke="url(#login-fold-core)" strokeLinecap="round" strokeWidth="2" />
        <path d="M45 68C56 75 66 82 77 91" stroke="hsl(var(--login-mark-cream) / 0.36)" strokeLinecap="round" strokeWidth="1.4" />
        <path d="M43 84C55 90 67 98 78 108" stroke="hsl(var(--login-mark-cream) / 0.3)" strokeLinecap="round" strokeWidth="1.2" />
        <path d="M47 101C58 106 69 114 79 124" stroke="hsl(var(--login-mark-cream) / 0.24)" strokeLinecap="round" strokeWidth="1.1" />
        <path d="M113 68C102 76 92 84 83 94" stroke="hsl(var(--login-accent) / 0.16)" strokeLinecap="round" strokeWidth="1.1" />
        <path d="M80 137C75 130 69 124 62 119" stroke="hsl(var(--login-accent-strong) / 0.82)" strokeLinecap="round" strokeWidth="1.8" />
      </svg>
    </div>
  );
}
...
function FoldLineField() {
  return (
    ...
    >
      <defs>
        <linearGradient id="login-fold-line-horizontal" x1="116" x2="812" y1="92" y2="92" gradientUnits="userSpaceOnUse">
        ...
        <linearGradient id="login-fold-line-vertical" x1="464" x2="464" y1="92" y2="364" gradientUnits="userSpaceOnUse">
          <stop stopColor="hsl(var(--login-accent) / 0.16)" />
          <stop offset="0.48" stopColor="hsl(var(--login-accent) / 0.1)" />
          <stop offset="1" stopColor="hsl(var(--login-accent) / 0)" />
        </linearGradient>
        <linearGradient id="login-fold-line-ray" x1="464" x2="116" y1="92" y2="306" gradientUnits="userSpaceOnUse">
          <stop stopColor="hsl(var(--login-accent) / 0.14)" />
          <stop offset="1" stopColor="hsl(var(--login-accent) / 0)" />
        </linearGradient>
      </defs>
      <path d="M116 92H812" stroke="url(#login-fold-line-horizontal)" strokeWidth="1" />
      <path d="M464 92V364" stroke="url(#login-fold-line-vertical)" strokeWidth="1" />
      <path d="M464 92L244 224L116 306" stroke="url(#login-fold-line-ray)" strokeWidth="1" />
      <path d="M464 92L684 224L812 306" stroke="url(#login-fold-line-ray)" strokeWidth="1" />
      <path d="M464 92L318 178L208 242" stroke="hsl(var(--login-foreground) / 0.045)" strokeWidth="1" />
      <path d="M464 92L610 178L720 242" stroke="hsl(var(--login-foreground) / 0.045)" strokeWidth="1" />
      <path d="M464 92L358 250L284 360" stroke="hsl(var(--login-accent) / 0.07)" strokeWidth="1" />
      <path d="M464 92L570 250L644 360" stroke="hsl(var(--login-accent) / 0.07)" strokeWidth="1" />
    </svg>
  );
}
  ...
  return (
    ...
    >
      ...
      <span className="relative z-10 flex w-full items-center justify-between px-1.5">
        <span className={cn("flex h-7 w-7 items-center justify-center transition-opacity", isDark ? "opacity-55" : "opacity-100")}>
          <svg className="h-4 w-4 text-[hsl(var(--login-accent))]" fill="none" viewBox="0 0 24 24">
            <circle cx="12" cy="12" r="4.5" stroke="currentColor" strokeWidth="1.8" />
            <path d="M12 2.75V5M12 19V21.25M21.25 12H19M5 12H2.75M18.54 5.46L16.95 7.05M7.05 16.95L5.46 18.54M18.54 18.54L16.95 16.95M7.05 7.05L5.46 5.46" stroke="currentColor" strokeLinecap="round" strokeWidth="1.8" />
          </svg>
        </span>
        <span className={cn("flex h-7 w-7 items-center justify-center transition-opacity", isDark ? "opacity-100" : "opacity-55")}>
          <svg className="h-4 w-4 text-[hsl(var(--login-accent))]" fill="none" viewBox="0 0 24 24">
            <path d="M19.25 14.25C18.15 14.9 16.87 15.25 15.5 15.25C11.36 15.25 8 11.89 8 7.75C8 6.39 8.35 5.1 9 4C5.54 5.18 3 8.46 3 12.25C3 17.02 6.98 21 11.75 21C15.54 21 18.82 18.46 20 15C19.76 14.77 19.51 14.52 19.25 14.25Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.8" />
          </svg>
        </span>
      </span>
    </button>

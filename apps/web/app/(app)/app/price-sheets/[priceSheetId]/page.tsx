...
export default async function PriceSheetEditPage({
  params,
}: PriceSheetEditPageProps) {
        ...
        <div className="relative space-y-7">
          ...
          <PageHeader
            ...
            actions={
              <div className="flex flex-wrap items-center gap-2">
                <form
                  action={duplicatePriceSheetAction.bind(null, priceSheet.id)}
                >
                  <Button
                    className="bg-card/70 w-full sm:w-auto"
                    size="sm"
                    type="submit"
                    variant="outline"
                  >
          ...
          <div className="grid gap-5 xl:grid-cols-[1fr,320px]">
            ...
            <div className="space-y-4">
              ...
              <Card className="border-border/75 bg-card/95 before:via-primary/25 dark:bg-card/90 dark:before:via-primary/35 relative overflow-hidden rounded-[1.65rem] shadow-[0_18px_55px_rgba(15,23,42,0.045)] before:absolute before:inset-x-0 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:to-transparent dark:shadow-[0_18px_60px_rgba(0,0,0,0.22)]">
                ...
                <CardContent className="space-y-4 p-5">
                  <div className="flex flex-wrap items-center gap-2">
                    <PriceSheetStatusBadge
                      ...
                      status={priceSheet.status}
                    />
                    <span className="border-border/70 bg-card/80 text-muted-foreground rounded-full border px-2.5 py-1 text-xs">
                      {priceSheet.itemCount} {messages.priceSheets.itemsCount}
                    </span>
                  </div>

                  <div className="border-border/70 bg-background/85 dark:bg-background/55 grid grid-cols-2 gap-1 rounded-2xl border p-1">
                    <span
                      className={cn(
                  ...
                  <div className="space-y-2.5 text-sm">
                    <div className="border-border/60 bg-background/70 text-muted-foreground dark:bg-background/45 flex items-start justify-between gap-3 rounded-2xl border px-3 py-2.5">
                      <span>{messages.priceSheets.slugLabel}</span>
                      <span className="text-foreground font-mono text-xs uppercase tracking-[0.18em]">
                        /{priceSheet.slug}
                      </span>
                    </div>
                    <div className="border-border/60 bg-background/70 text-muted-foreground dark:bg-background/45 flex items-start justify-between gap-3 rounded-2xl border px-3 py-2.5">
                  ...
                  <form
                    action={setPriceSheetStatusAction.bind(
                      ...
                      priceSheet.id,
                      nextStatus,
                      `/app/price-sheets/${priceSheet.id}`,
                    )}
                  >
              ...
              <Card className="border-destructive/20 bg-card/80 dark:bg-card/70 relative overflow-hidden rounded-[1.45rem] shadow-[0_12px_30px_rgba(15,23,42,0.03)]">
                <CardContent className="p-4">
                  <DeletePriceSheetConfirmation
                    action={deletePriceSheetAction.bind(
                      null,
                      priceSheet.id,
                      "/app/price-sheets",
                    )}
                    ...
                    confirmButtonLabel={
                      messages.priceSheets.deleteConfirmButton
                    }
